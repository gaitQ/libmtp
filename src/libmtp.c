...
int LIBMTP_Custom_Operation_Get_arg(LIBMTP_mtpdevice_t *device, uint16_t code, uint32_t arg, unsigned char **data, unsigned int *size)
{
  PTPParams *params = (PTPParams *) device->params;
  PTPContainer ptp;
  uint16_t ret;
  ...
  if (ret != PTP_RC_OK) {
    add_ptp_error_to_errorstack(device, ret, "LIBMTP_Custom_Operation_Get_arg(): failed to execute operation.");
    return -1;
  }
